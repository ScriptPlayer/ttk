...
player.removeVirtualEquip(card);
	...
	replaceHandcards: function () {
		"step 0";
		console.log("replaceHandcards step 0===============");
	...
	replaceHandcardsOL: function () {
		"step 0";
		var send = function () {
			game.me.chooseBool("是否使用手气卡？剩余次数：");
			game.resume();//不用这个不会显示置换手牌的按钮
		};
		var sendback = function (result, player) {
			if (result && result.bool && player.sqkCount!==0) {
				/*otherPile主要是针对那些用专属牌堆，不从一般牌堆摸牌的角色（如陈寿），该属性目前只有两个键值对，且都为函数
				 *getCards函数与获得牌相关，只传入要获得的牌数num作为参数
				 *discard与手气卡换牌后弃置牌相关，只传入要弃置的牌card作为参数
				 */
				...
				//手气卡-1
				player.sqkCount=player.sqkCount-1;
				if(player==game.me&&player.sqkCount>0){
					//console.log('房主点击了更换');
					event.goto(0)
				}
			}else{
				console.log(player.nickname+"：取消使用手气卡");
				player.sqkCount=0;
			}
			player.thinkUseSqk=false;
			console.log(player.nickname+"sqkCount============"+player.sqkCount);
		};

		if(game.me.sqkCount===7){
			console.log('保证手气卡事件的setInterval只执行一次');
			var sqkInterval=setInterval(() => {
				let condition=false;
				for (let i = 0; i < event.players.length; i++) {
					if (event.players[i].isOnline()&&event.players[i].sqkCount>0){
						condition=true;
						if(!event.players[i].thinkUseSqk){
							event.otherSqkFinish=false;
							event.players[i].thinkUseSqk=true;
							event.withol = true;
							event.players[i].send(send);
							event.players[i].wait(sendback);
						}
					}
					else if (event.players[i] == game.me&&event.players[i].sqkCount>0) {
						condition=true;
					}
				}
				if(condition===false){
					console.log('condition===false');
					clearInterval(sqkInterval);
					setTimeout(() => {
						game.me.unwait();
						game.resume();
					}, 500);
		...
		event.withme=true;
		game.me.chooseBool("是否使用手气卡？剩余次数："+game.me.sqkCount);
		game.me.wait(sendback);
		"step 1";
		console.log("step:1",event.withme);
		if (event.withme) {
	...
	//原版
	replaceHandcardsOLOriginal: function () {
		"step 0";
		var send = function () {
			game.me.chooseBool("是否置换手牌？");
			game.resume();
	...
	chooseBool: function () {
		...
		if (event.isMine()) {
			//console.log('isMine???????',event.isMine());
			...
			if (event.dialog) {
				...
				if(event.prompt==='是否使用手气卡？剩余次数：'&&event.player.sqkCount==null){
					event.player.sqkCount=7;
					event.prompt='是否使用手气卡？剩余次数：7';
					event.player.sqkCount--;
				}else if(event.prompt==='是否使用手气卡？剩余次数：'){
					event.prompt='是否使用手气卡？剩余次数：'+event.player.sqkCount;
					event.player.sqkCount--;
				}
				event.dialog = ui.create.dialog(event.prompt);
				if (event.prompt2) {
			...
			//console.log('event.isOnline()??????',event.isOnline());
			event.send();
		} else {
			event.result = "ai";
		}
