...
export class Player extends HTMLDivElement {
	...
	*iterableGetGainableCards(player, arg1, arg2) {
	...
	wait(callback) {
		if (lib.node) {
			...
			if (this.ws && !this.ws.closed) {
				...
				// if(_status.event.name==='replaceHandcards'){
				// 	console.log("_status.event.name==='replaceHandcards'");
				// 	time=5000;//5秒换牌时间
				// }
	...
	showTimer(time) {
		...
		// if(_status?.event?.name==='chooseBool'&&this?.nickname!=null&&this?.nickname!==''){
		// 	console.log("setTime***************");
		// 	time=5080;
		// }
