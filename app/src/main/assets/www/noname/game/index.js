...
export class Game extends GameCompatible {
	...
	replaceHandcards(...args) {
		...
		//给在线玩家设置sqkCount=7，确认使用手气卡时-1，取消时设置为0，根据sqkCount>0判断
		for (const player of next.players) {
			if(player.nickname!=null&&player.nickname.trim()!==''){
				player.sqkCount=7;
			}
		}
		if (_status.connectMode) {
			console.log('联机模式进入replaceHandcardsOL');
	...
	countDown(time, onEnd) {
		...
		// @ts-expect-error ignore
		time = parseInt(time);
		//可以加点判断条件，phaseNumber=0；还有就是可以设置sqkCount的时机提前，用sqkCount判断
		// if(_status?.event?.name==='chooseBool'&&_status?.event?.player?.nickname!=null
		// 	&&_status?.event?.prompt?.includes("是否使用手气卡？")){
		// 	console.log("是否使用手气卡？只有5秒思考时间");
		// 	// time=5;
		// }
		if (!time) {
			return;
