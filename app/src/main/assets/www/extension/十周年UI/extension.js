	...
	return {
					...
					ride.ui = {
						...
						updatej(player) {
							...
							for (var i = 0; i < judges.length; i++) {
								...
								if (_status.connectMode) {
									...
									if(bgMark.trim()==='粮'||bgMark.trim()==='乐'||bgMark.trim()==='电'){
										//console.log('bgMark=======123',bgMark);
										judges[i].node.judgeMark.node.judge.innerHTML = '';
									}
								}
							}
						},
						...
						create: {
							...
							dialog() {
								...
								// //结束对话框，一般都是第二个dialog才是，小概率第一个就是。
								// let dialogEle2=document.getElementsByClassName(dialog.className)[1];
								// //console.log(dialogEle?.innerHTML);
								// //console.log(dialogEle2?.innerHTML);
								// let b1=dialogEle?.innerHTML.includes('选择角色');
								// let b2=dialogEle?.innerHTML.includes('选择身份');
								// let b3=dialogEle?.innerHTML.includes('战斗胜利');//小概率
								// let b4=dialogEle?.innerHTML.includes('战斗失败');//小概率
								// let b5=dialogEle2?.innerHTML.includes('战斗胜利');
								// let b6=dialogEle2?.innerHTML.includes('战斗失败');
								// //console.log(b1);
								// //console.log(b2);
								// //console.log(notouchscroll)//false
								// if((b1&&b2)||b5||b6||b3||b4){//确保dialog同时有选择角色和选择身份 或者 是结束的dialog
							...
							if (typeof node.popupNumber == "number") {
								...
								if (popupNum < 0) {
									if(popupNum<=-4){
										...
										game.playAudio("../extension", decadeUI.extensionName, "audio/wanjunqushou.mp3");
									}
									if(popupNum===-3){
										decadeUI.animation.playSpine(
											{
									...
									//是这里，受击动画
									let randomShouJiName = ['dao', 'jian', 'fuzi'].randomGet();
									switch (node.nature) {
										case "thunder":
											decadeUI.animation.playSpine(
												{
													name: randomShouJiName,
													action: "play",
												},
												{
													scale: 0.8,
		...
		precontent() {
			...
			game.showChatWordBackgroundX = function () {
				...
				window.chatBg.style.transition = "all 0.1s";
